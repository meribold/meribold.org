...
At some point I noticed that google.com and google.de don't give the same results.
<!--
There currently are four results each but one page is different: google.com has
...
On 22., at least one more page could be found by [adding keywords][15].
<!-- (Doesn't work on google.com.) -->
...
This section is a gallery of tricky content that tests how the CSS copes.

The following code block has quite long lines.  It may extend to the right of where normal
text is wrapped if it needs the space and the viewport is wide enough.

```
int this_is_a_code_block_with_quite_long_lines = its_purpose_is_to_test_how_the_css_copes_with_something_like_that();
printf("%d", this_is_a_code_block_with_quite_long_lines);
```

Here's another code block.  This one has a pretty ridiculous line length; it may extend to
the very right of the viewport and still have a horizontal scrollbar.

```
float in_practice_its_probably_a_good_idea_to_hard_wrap_at_some_point_though = this_is_a_constant_with_a_name_that_is_much_too_long_unfortunately + 123456789.0 / 2.0;
```
